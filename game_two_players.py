...
def log(score, player_scores):
    t=len(player_scores)%num_players 
    if t==0:
        t=num_players
    if player_scores[-1][1] == "Player 2":
        listbox.insert(tkinter.END, f"")
    log_file.write(f"Player {(t)}: {score}\n")
...
def click(event):
    ...
    t=len(player_scores)%num_players + 1
    player_scores.append([score, f"Player {t}"])
    update_scores()
    log(score, player_scores)
    

# Function to update score display
def update_scores():
    listbox.insert(tkinter.END, f"{player_scores[-1][1]}: {player_scores[-1][0]}")    
    score_player_1 = sum([x[0] for x in player_scores if x[1] == 'Player 1'])
    score_player_2 = sum([x[0] for x in player_scores if x[1] == 'Player 2'])
    
    player_1_score_display = tkinter.Label(root, text=f"Player 1 Score\n {score_player_1}", font=("Helvetica", 16))
    player_1_score_display.place(x=120, y=20)
    player_1_score_display.config()
    
    player_2_score_display = tkinter.Label(root, text=f"Player 2 Score\n {score_player_2}", font=("Helvetica", 16))
    player_2_score_display.config()
    player_2_score_display.place(x=750, y=20)
...
player_1_score_display = tkinter.Label(root, text="Player 1 Score\n 0", font=("Helvetica", 16))
player_1_score_display.place(x=120, y=20)

# Display the bot score
player_2_score_display = tkinter.Label(root, text="Player 2 Score\n 0", font=("Helvetica", 16))
player_2_score_display.place(x=750, y=20)
